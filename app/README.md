...
- **CDC Growth Charts (2–20 years)** — Rebuilt using official percentile curves (P3 through P97)
- **"CETAK" (Print to Chart)** — High-fidelity rendering of measurement data on official CDC-style chart templates for clinical printing
...
│   ├── cdc-print-chart.tsx # Canvas-based clinical print component
│   ├── powered-by.tsx      # SEHA branding
│   └── theme-provider.tsx  # Gender-based theming
├── lib/
│   ├── growth-standards.ts # WHO/CDC Z-Score calculations
│   ├── cdc-percentile-data.ts # Official CDC percentile data
│   ├── who-data.ts         # WHO LMS data
│   ├── cdc-data.ts         # CDC LMS data (LMS only)
